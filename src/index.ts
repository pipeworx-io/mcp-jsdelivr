...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'JSDelivr Data API');
}
...
const tools: McpToolExport['tools'] = [
  ...
  {
    name: 'list_npm_files',
    description: 'jsDelivr Data API — the file tree of one published npm package version (latest by default) as jsDelivr indexes it for CDN delivery. Returns the directory structure with each file\'s path, byte size, content hash and type, read from jsDelivr\'s package index. Answers which files, entry points and build outputs an npm package version actually ships.',
    inputSchema: {
      type: 'object',
      properties: {
        ...
        version: {
          type: 'string',
          description:
            'Specific published version, e.g. "4.17.21". Omit this argument entirely to get the ' +
            'latest published version — do not pass the literal string "latest"; jsDelivr\'s API ' +
            'rejects that as an unresolvable version and returns 404.',
        },
      },
      required: ['package_name'],
    },
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    ...
    case 'list_npm_files': {
      const pkg = encodePkg(reqStr(args, 'package_name', '"react"'));
      const requested = (args.version as string | undefined)?.trim();
      // jsDelivr's file-tree endpoint wants a CONCRETE version — `<pkg>@latest` 404s with
      // "Couldn't find version latest... use a specific version number, not a tag" (verified
      // live, fleet #2070). So when no version is given, resolve "latest" to a real version
      // number first via the /resolved endpoint, then fetch the tree for that version.
      const version = requested || (await resolveVersion(pkg, 'latest'));
...
function encodePkg(pkg: string): string {
  // A scoped name is ONE path segment to jsDelivr, so the slash inside it must be
  // escaped too. Splitting on "/" and encoding each half produced "%40angular/core"
  // — the `@` escaped, the `/` left literal — and that half-escaped form is the one
  // shape jsDelivr rejects. Measured 2026-09-16 against every endpoint family this
  // pack uses (/packages/npm/<pkg>, /<pkg>/resolved, /<pkg>@<version>,
  // /stats/packages/npm/<pkg>): raw "@angular/core" -> 200, fully-escaped
  // "%40angular%2Fcore" -> 200, half-escaped "%40angular/core" -> 400 on all four.
  // The families do NOT disagree (contra fleet #2104's premise); the encoder was
  // emitting a form none of them accept. Fully escaped is what jsDelivr itself puts
  // in the `links.self` of its own responses, so that is what we send.
  return encodeURIComponent(pkg);
}

/** Resolve a semver range/tag (e.g. "latest") to a concrete published version string. */
async function resolveVersion(encodedPkg: string, range: string): Promise<string> {
  const resolved = (await jdGet(`/packages/npm/${encodedPkg}/resolved?specifier=${encodeURIComponent(range)}`)) as {
    version?: string;
  };
  if (!resolved?.version) throw new Error(`JSDelivr: could not resolve "${range}" to a version.`);
